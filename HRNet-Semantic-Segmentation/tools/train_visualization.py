...
def visualize_segmentation(model, config, image_path, output_path):
    ...
    input_image = torch.from_numpy(input_image).unsqueeze(0)  # Add batch dimension
    input_image = input_image.float()  # 입력 데이터를 Float 타입으로 변환
    ...
    for class_idx in range(config.DATASET.NUM_CLASSES):
        mask[output == class_idx] = [class_idx * 10, 0, 255 - class_idx * 10]

    # 원본 이미지와 합성
    alpha = 0.5
    masked_image = cv2.addWeighted(image, 1 - alpha, mask, alpha, 0)
...
def main():
    ...
    if args.local_rank <= 0:
        ...
        # 세그멘테이션 결과 시각화
        image_paths = [
            'C:/Users/yujin/HRNet/HRNet-Semantic-Segmentation/data/pole/images/val/DSC_0005.JPG',
            'C:/Users/yujin/HRNet/HRNet-Semantic-Segmentation/data/pole/images/val/DSC_1089.JPG'
        ]
        for image_path in image_paths:
            output_path = image_path.replace('images/val', 'output').replace('.JPG', '_segmentation_result.png')
            masked_image = visualize_segmentation(model, config, image_path, output_path)
